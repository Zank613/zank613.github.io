...
export class IdDbApp {
    ...
    render(ctx, rect) {
        ...
        if (!selectedId) {
            ...
            ctx.fillText("Use Citizen_DB to choose a candidate first.", 16, 56);
            ctx.restore();
            return;
        }
        ...
        ctx.fillText(`${c.name} ${c.surname}`, 16, y);
        y += 22;
        ...
        ctx.fillStyle = "#ffffff";
        ctx.fillText(
            `Height: ${c.physical.height_cm} cm, Weight: ${c.physical.weight_kg} kg`,
            32, y
        ); y += 16;
        ctx.fillText(
            `Age: ${c.physical.age}, Eyes: ${c.physical.eye_color}, Hair: ${c.physical.hair_color}`,
            32, y
        ); y += 18;
